...
export default function NoVNCPlayer({
    host,
    port,
    sessionId,
    gameId,
    ...
    // NoVNC WebSocket URL (via Orchestrator proxy)
    const wsUrl = `ws://localhost:3012/vnc/${host}/${port}`;

    useEffect(() => {
        setConnectionStatus(`Connecting to ${host}:${port}...`);
        ...
        testWs.onopen = () => {
            ...
            setConnectionStatus('WebSocket Connected - Waiting for VNC handshake...');
        };
                ...
                <div className="relative w-full h-full">
                    ...
                    <div className="w-full h-full bg-black flex items-center justify-center">
                        <div className="text-center max-w-2xl px-8">
                            ...
                            <h2 className="text-white text-3xl font-bold mb-4">{connectionStatus}</h2>
                            <div className="bg-white/5 rounded-lg p-6 text-left">
                                <p className="text-white/80 text-sm mb-2">
                                    <span className="text-emerald-400">✓</span> Interface Strike loaded
                                </p>
                                <p className="text-white/80 text-sm mb-2">
                                    <span className="text-emerald-400">✓</span> WebSocket proxy ready
                                </p>
                                <p className="text-white/80 text-sm mb-2">
                                    <span className="text-yellow-400">⟳</span> Connecting to VNC: {host}:{port}
                                </p>
                            ...
                            <div className="mt-8 p-4 bg-red-500/10 border border-red-500/20 rounded-lg">
                                <p className="text-red-400 text-sm font-semibold mb-2">⚠️ NoVNC Library Issue</p>
                                <p className="text-white/60 text-xs">
                                    NoVNC CDN loading failed. We need to implement a custom VNC client or use a different approach.
                                </p>
                                <p className="text-white/60 text-xs mt-2">
                                    Next step: Implement WebRTC-based streaming or use Apollo + custom WebRTC proxy.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="absolute top-0 left-0 right-0 p-6 bg-gradient-to-b from-black/60 to-transparent pointer-events-none">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-white/60 text-sm">Cloud Gaming Session</p>
                            </div>
                        </div>
                    </div>
