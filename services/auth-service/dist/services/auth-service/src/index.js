...
app.post('/api/auth/v1/oauth/:provider/callback', {
    ...
    try {
        ...
        return reply.redirect(`${frontendUrl}/auth/callback?token=${tokenPair.accessToken}&refreshToken=${tokenPair.refreshToken}`);
    }
    catch (error) {
        app.log.error({ err: error }, 'Error in OAuth callback');
        return reply.status(500).send((0, shared_utils_1.errorResponse)(shared_utils_1.ErrorCodes.INTERNAL_ERROR, 'Failed to process OAuth callback'));
    }
});
