...
const VANITY_URL = 'domeniconicastro';

async function main() {
    console.log(`Resolving vanity URL: ${VANITY_URL}...`);

    try {
        // 1. Resolve Vanity URL
        const resolveUrl = `http://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001/?key=${STEAM_API_KEY}&vanityurl=${VANITY_URL}`;
        const resolveRes = await fetch(resolveUrl);
        const resolveData = await resolveRes.json() as any;

        if (resolveData.response.success !== 1) {
            console.error('Failed to resolve vanity URL:', resolveData);
            return;
        }

        const steamId64 = resolveData.response.steamid;
        console.log(`Resolved SteamID64: ${steamId64}`);

        // 2. Get Owned Games
        const ownedUrl = `http://api.steampowered.com/IPlayerService/GetOwnedGames/v1/?key=${STEAM_API_KEY}&steamid=${steamId64}&include_appinfo=true&include_played_free_games=1&format=json`;
